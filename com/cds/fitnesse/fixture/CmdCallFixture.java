...
//			serv = getAS400(SERV, dbConn.getUser(), dbConn.getPassword());
			...
			e.printStackTrace();
			thisCall.add(new CommandExecution(command, "Could not create AS400 object @ getAS400() - AS400SecurityException"));
...
//			thisCall.get(0).setCmd("Could not create AS400 object @ getAS400() - AS400SecurityException");
		...
		try{
			...
			for (int i = 0; i < messagelist.length; ++i){
				...
				thisCall.add(new CommandExecution(command, messagelist[i].getText()));
			}	 
...
//	        thisCall.get(0).setCmd(command);
//	        thisCall.get(0).setReturnMsg(returnMsg);
	        thisCall.add(new CommandExecution(command, "Command failed - caught exception on cmd.run()"));
